...
public class Boid : MonoBehaviour
{
    ...
    neighbors = new List<Boid>(); // 3
    collisionRisks = new List<Boid>();

    // Make this.transform a child of the Boids GameObject
    this.transform.parent = GameObject.Find("Boids").transform; // 4
    ...
    List<Boid> neighbors = GetNeighbors(this); // 6
    ...
    Vector3 neighborCenterOffset = GetAveragePosition( neighbors ) -
    ...
    velocity = (1-
    BoidSpawner.S.velocityLerpAmt)*velocity + BoidSpawner.S.velocityLerpAmt*newVelocity;
    ...
    public List<Boid> GetNeighbors(Boid boi) { // 8
    ...
    public Vector3 GetAveragePosition( List<Boid> someBoids ) { // 9
    ...
    public Vector3 GetAverageVelocity( List<Boid> someBoids ) {{ // 10
    Vector3 sum = Vector3.zero;
    foreach (Boid b in someBoids) {
    sum += b.velocity;
    }
